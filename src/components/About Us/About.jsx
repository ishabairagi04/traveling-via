...
image: "/avatar1.jpg",
rating: 5,
text: `This travel agency made my dream trip a reality! The attention to detail and custom itinerary were amazing.
Highly recommended!`,
},
{
name: "James Peterson",
...
image: "/avatar2.jpg",
rating: 5,
text: `From the moment I booked till I returned, everything was smooth. Exceptional customer service and very affordable
packages.`,
},
{
name: "Sophia Lee",
...
image: "/avatar4.jpg",
rating: 5,
text: `I use their service for every international trip now. It's reliable, and I always get great deals and
recommendations.`,
},
{
name: "Emily Johnson",
...
{
icon:
<FaGlobeAsia className="text-yellow-500 text-3xl" />,
...
<section className="mt-16 sm:mt-20">
    ...
    <section className="px-4 md:px-16 py-12">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}
                transition={{ duration: 0.6 }} className="flex flex-col gap-6">
                <div className="relative rounded-xl shadow-md">
                    <img src="/about_2.webp" alt="Travel" className="w-full h-64 object-cover rounded-xl" />

                    {/* Circle */}
                    <motion.div initial={{ scale: 0 }} whileInView={{ scale: 1 }} transition={{ duration: 0.5, delay:
                        0.3 }} className="absolute left-0 top-1/2 transform -translate-x-1/2 -translate-y-1/2">
                        <div
                ...
                {/* Features */}
                <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={{ visible: {
                    ...
                    <motion.div key={i} variants={{ hidden: { opacity: 0, y: 30 }, visible: { opacity: 1, y: 0 }, }}
                        transition={{ duration: 0.5 }} className="flex flex-col items-start gap-3">
                        {icons[i]}
                        <div>
                ...
                {/* Booking CTA */}
                <motion.div initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{
                    duration: 0.5, delay: 0.3 }} viewport={{ once: true }}
    ...
    <section className="bg-[#d8a34d] w-full bg-opacity-30 py-16 px-4 ">
        <motion.div className="max-w-7xl mx-auto text-center" initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1,
            y: 0 }} transition={{ duration: 0.7 }} viewport={{ once: true }}>
            <motion.h2 className="text-3xl sm:text-4xl font-bold text-white mb-2" initial={{ opacity: 0, y: -20 }}
                whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} viewport={{ once: true }}>
                Why choose Via Brahman
            </motion.h2>
            <motion.p className="text-white mb-12" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{
                delay: 0.3, duration: 0.5 }} viewport={{ once: true }}>
            ...
            <motion.div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6" initial="hidden"
                whileInView="visible" viewport={{ once: true }} variants={{ hidden: {}, visible: { transition: {
                staggerChildren: 0.2, }, }, }}>
    ...
    <section className="bg-white py-20 px-4">
        <div className="max-w-7xl mx-auto text-center">
            <motion.h2 initial={{ opacity: 0, y: -30 }} whileInView={{ opacity: 1, y: 0 }} transition={{ duration: 0.6
                }} viewport={{ once: true }} className="text-3xl sm:text-4xl font-bold text-gray-800 mb-4">
            ...
            <Swiper modules={[Navigation, Pagination]} navigation pagination={{ clickable: true }} spaceBetween={30}
                breakpoints={{ 640: { slidesPerView: 1 }, 768: { slidesPerView: 2 }, 1024: { slidesPerView: 3 }, }}>
                {testimonials.map((t, index) => (
                <SwiperSlide key={index}>
                    <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} transition={{
                        duration: 0.6, delay: index * 0.1 }} viewport={{ once: true }}
    ...
    <section className="bg-[#fff8f0] py-20 px-4">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start gap-12">
            {/* Left Side */}
            <motion.div className="w-full md:w-1/2" initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }}
                ...
                <div className="flex items-center gap-4">
                    ...
                    <button
                        className="bg-yellow-500 text-white font-semibold px-6 py-2 rounded-full hover:bg-yellow-600 transition duration-300">
                        Booking Now
                    </button>
                </div>
            </motion.div>
            ...
            {/* Right Side Cards */}
            <motion.div className="w-full md:w-1/2 grid grid-cols-1 sm:grid-cols-2 gap-6" initial="hidden"
                whileInView="visible" variants={{ hidden: {}, visible: { transition: { staggerChildren: 0.2, }, }, }}
                viewport={{ once: true }}>
                {steps.map((step, idx) => (
